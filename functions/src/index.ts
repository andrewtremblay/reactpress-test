...
export const cleanupUser = functions.auth.user().onDelete(async (user) => {
  ...
  const paymetsMethodsSnapshot = await dbRef
    ...
    .get();
  paymetsMethodsSnapshot.forEach((snap: any) => batch.delete(snap.ref));
  const paymentsSnapshot = await dbRef
    .doc(user.uid)
    .collection("payments")
    .get();
  paymentsSnapshot.forEach((snap: any) => batch.delete(snap.ref));
...
function reportError(err: any, context: any = {}) {
